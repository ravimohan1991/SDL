...
macro(CheckOSS)
  if(SDL_OSS)
    ...
    if(OSS_FOUND)
      set(HAVE_OSS TRUE)
      file(GLOB OSS_SOURCES ${CMAKE_CURRENT_SOURCE_DIR}/src/audio/dsp/*.c)
...
macro(CheckALSA)
  if(SDL_ALSA)
    ...
    if(HAVE_LIBASOUND)
      set(HAVE_ALSA TRUE)
      file(GLOB ALSA_SOURCES ${CMAKE_CURRENT_SOURCE_DIR}/src/audio/alsa/*.c)
...
macro(CheckPipewire)
    if(SDL_PIPEWIRE)
        ...
        if(PKG_PIPEWIRE_FOUND)
            ...
            file(GLOB PIPEWIRE_SOURCES ${CMAKE_CURRENT_SOURCE_DIR}/src/audio/pipewire/*.c)
...
macro(CheckPulseAudio)
  if(SDL_PULSEAUDIO)
    ...
    if(PKG_PULSEAUDIO_FOUND)
      ...
      file(GLOB PULSEAUDIO_SOURCES ${CMAKE_CURRENT_SOURCE_DIR}/src/audio/pulseaudio/*.c)
...
macro(CheckJACK)
  if(SDL_JACK)
    ...
    if(PKG_JACK_FOUND)
      set(HAVE_JACK TRUE)
      file(GLOB JACK_SOURCES ${CMAKE_CURRENT_SOURCE_DIR}/src/audio/jack/*.c)
...
macro(CheckESD)
  if(SDL_ESD)
    ...
    if(PKG_ESD_FOUND)
      set(HAVE_ESD TRUE)
      file(GLOB ESD_SOURCES ${CMAKE_CURRENT_SOURCE_DIR}/src/audio/esd/*.c)
...
macro(CheckARTS)
  if(SDL_ARTS)
    ...
    if(ARTS_CONFIG)
      ...
      file(GLOB ARTS_SOURCES ${CMAKE_CURRENT_SOURCE_DIR}/src/audio/arts/*.c)
...
macro(CheckNAS)
  if(SDL_NAS)
    ...
    if(HAVE_NAS_H AND D_NAS_LIB)
      set(HAVE_NAS TRUE)
      file(GLOB NAS_SOURCES ${CMAKE_CURRENT_SOURCE_DIR}/src/audio/nas/*.c)
...
macro(CheckSNDIO)
  if(SDL_SNDIO)
    ...
    if(HAVE_SNDIO_H AND D_SNDIO_LIB)
      set(HAVE_SNDIO TRUE)
      file(GLOB SNDIO_SOURCES ${CMAKE_CURRENT_SOURCE_DIR}/src/audio/sndio/*.c)
...
macro(CheckFusionSound)
  if(FUSIONSOUND)
    ...
    if(PKG_FUSIONSOUND_FOUND)
      ...
      file(GLOB FUSIONSOUND_SOURCES ${CMAKE_CURRENT_SOURCE_DIR}/src/audio/fusionsound/*.c)
...
macro(CheckX11)
  if(SDL_X11)
    ...
    if(X11_LIB)
      ...
      file(GLOB X11_SOURCES ${CMAKE_CURRENT_SOURCE_DIR}/src/video/x11/*.c)
...
macro(CheckWayland)
  if(SDL_WAYLAND)
    ...
    if(WAYLAND_FOUND)
      ...
      file(GLOB WAYLAND_SOURCES ${CMAKE_CURRENT_SOURCE_DIR}/src/video/wayland/*.c)
      ...
      file(GLOB WAYLAND_PROTOCOLS_XML RELATIVE "${CMAKE_CURRENT_SOURCE_DIR}/wayland-protocols/" "${CMAKE_CURRENT_SOURCE_DIR}/wayland-protocols/*.xml")
      foreach(_XML ${WAYLAND_PROTOCOLS_XML})
        string(REGEX REPLACE "\\.xml$" "" _PROTL "${_XML}")
        WaylandProtocolGen("${WAYLAND_SCANNER}" "${WAYLAND_SCANNER_CODE_MODE}" "${CMAKE_CURRENT_SOURCE_DIR}/wayland-protocols/${_XML}" "${_PROTL}")
      endforeach()
...
macro(CheckCOCOA)
  if(SDL_COCOA)
    ...
    if(HAVE_COCOA)
      file(GLOB COCOA_SOURCES ${CMAKE_CURRENT_SOURCE_DIR}/src/video/cocoa/*.m)
...
macro(CheckDirectFB)
  if(SDL_DIRECTFB)
    ...
    if(PKG_DIRECTFB_FOUND)
      ...
      file(GLOB DIRECTFB_SOURCES ${CMAKE_CURRENT_SOURCE_DIR}/src/video/directfb/*.c)
...
macro(CheckVivante)
  if(SDL_VIVANTE)
    ...
    if(HAVE_VIVANTE_VDK OR HAVE_VIVANTE_EGL_FB)
      ...
      file(GLOB VIVANTE_SOURCES ${CMAKE_CURRENT_SOURCE_DIR}/src/video/vivante/*.c)
  ...
  if(SDL_THREADS AND SDL_PTHREADS)
    ...
    if(HAVE_PTHREADS)
      ...
      set(SOURCE_FILES ${SOURCE_FILES}
          ...
          ${CMAKE_CURRENT_SOURCE_DIR}/src/thread/pthread/SDL_sysmutex.c   # Can be faked, if necessary
          ${CMAKE_CURRENT_SOURCE_DIR}/src/thread/pthread/SDL_syscond.c    # Can be faked, if necessary
          ${CMAKE_CURRENT_SOURCE_DIR}/src/thread/pthread/SDL_systls.c
          )
      if(HAVE_PTHREADS_SEM)
        set(SOURCE_FILES ${SOURCE_FILES}
...
macro(CheckUSBHID)
  ...
  if(HAVE_USBHID)
    ...
    file(GLOB BSD_JOYSTICK_SOURCES ${CMAKE_CURRENT_SOURCE_DIR}/src/joystick/bsd/*.c)
...
macro(CheckHIDAPI)
  ...
  if(SDL_HIDAPI)
    ...
    if(HAVE_HIDAPI)
      if(ANDROID)
        list(APPEND SOURCE_FILES ${CMAKE_CURRENT_SOURCE_DIR}/src/hidapi/android/hid.cpp)
      endif()
      if(IOS OR TVOS)
        list(APPEND SOURCE_FILES ${CMAKE_CURRENT_SOURCE_DIR}/src/hidapi/ios/hid.m)
      ...
      if(SDL_JOYSTICK AND SDL_HIDAPI_JOYSTICK)
        ...
        file(GLOB HIDAPI_JOYSTICK_SOURCES ${CMAKE_CURRENT_SOURCE_DIR}/src/joystick/hidapi/*.c)
...
macro(CheckRPI)
  if(SDL_RPI)
    ...
    if(SDL_VIDEO AND HAVE_RPI)
      ...
      file(GLOB VIDEO_RPI_SOURCES ${CMAKE_CURRENT_SOURCE_DIR}/src/video/raspberry/*.c)
...
macro(CheckKMSDRM)
  if(SDL_KMSDRM)
    ...
    if(KMSDRM_FOUND AND HAVE_OPENGL_EGL)
      ...
      file(GLOB KMSDRM_SOURCES ${CMAKE_CURRENT_SOURCE_DIR}/src/video/kmsdrm/*.c)
